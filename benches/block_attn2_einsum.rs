...
impl Circuit<Fr> for MyCircuit {
    ...
    fn configure(cs: &mut ConstraintSystem<Fr>) -> Self::Config {
        ...
        let a = VarTensor::new_advice(cs, K, 1 * 4 * len * 16);

        let b = VarTensor::new_advice(cs, K, 1 * 4 * len * 16);

        let output = VarTensor::new_advice(cs, K, 1 * 4 * len * 16);
...
fn runmatmul(c: &mut Criterion) {
    let mut group = c.benchmark_group("block_attn2_einsum");
    ...
    for &len in [64, 80, 96, 112, 128].iter() {
        ...
        let mut a = Tensor::from((0..1 * 4 * len * len).map(|_| Value::known(Fr::random(OsRng))));
        a.reshape(&[1, 4, len, len]);

        // parameters
        let mut b = Tensor::from((0..1 * 4 * len * 16).map(|_| Value::known(Fr::random(OsRng))));
        b.reshape(&[1, 4, len, 16]);
