...
impl Circuit<Fr> for MyCircuit {
    ...
    fn configure(cs: &mut ConstraintSystem<Fr>) -> Self::Config {
        let len = unsafe { LEN };
        let embd = 64;
        let a = VarTensor::new_advice(cs, K, 1 * len * 64 * (embd/len));

        let b = VarTensor::new_advice(cs, K, 1 * len * 64 * (embd/len));

        let output = VarTensor::new_advice(cs, K, 1 * len * 64 * (embd/len));
...
fn runmatmul(c: &mut Criterion) {
    let mut group = c.benchmark_group("head_attn2_einsum");
    let params = gen_srs::<KZGCommitmentScheme<_>>(24);
    for &len in [4, 8, 16].iter() {
        unsafe {
            LEN = len;
        };
        let embd = 64;
        let mut a = Tensor::from((0..1 * len * 64 * 64).map(|_| Value::known(Fr::random(OsRng))));
        a.reshape(&[1, len, 64, 64]);

        // parameters
        let mut b = Tensor::from((0..1 * len * 64 * (embd/len)).map(|_| Value::known(Fr::random(OsRng))));
        b.reshape(&[1, len, 64, (embd/len)]);
