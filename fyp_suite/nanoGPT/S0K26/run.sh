...
RUST_LOG=debug cargo run --bin ezkl --release forward --data input.json --model network.onnx --output output.json  -K=26 -S=0 -B=7 | tee logs/forward_log.txt 

RUST_LOG=debug cargo run --bin ezkl --release setup -D output.json -M network.onnx --params-path=../../kzg26.params --vk-path=vk.key --pk-path=pk.key --circuit-params-path=circuit.params -K=26 -S=0 -B=7  | tee logs/setup_log.txt
